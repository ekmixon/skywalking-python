...
def start() -> None:
    ...
    try:
        ...
        cli_logger.debug(
            f"Failed to run the given user application command `{' '.join(args.command)}`, please make sure given command is valid."
        )
...
def dispatch(args: argparse.Namespace) -> None:
    ...
    cli_logger.debug(
        f"SkyWalking Python agent with CLI option '{cli_option}' and command {actual_command}"
    )
